...
namespace MetaCore::Engine {
    ...
    UnityEngine::Vector3 GetClampedEuler(UnityEngine::Quaternion rotation);
    ...
    void DisableAllBut(TransformWrapper parent, std::set<std::string> enabled, std::set<std::string> disabled = {});
    ...
    UnityEngine::Transform* FindRecursive(TransformWrapper parent, std::string name);
    ...
    std::string GetTransformPath(TransformWrapper parent, TransformWrapper child);
    ...
    void SetRelativeSiblingIndex(TransformWrapper child, TransformWrapper ref, int amount);
    ...
    ArrayW<UnityEngine::Color> ScalePixels(UnityEngine::Texture2D* texture, int width, int height, UnityEngine::Rect bounds = {});
    ...
    UnityEngine::Texture2D* ScaleTexture(UnityEngine::Texture2D* texture, int width, int height, UnityEngine::Rect bounds = {});
    /// @brief Scales the texture of a sprite to a new size, using basic interpolation
    /// @param texture The original texture
    /// @param width The width for the output, or -1 to leave unchanged
    /// @param height The height for the output, or -1 to leave unchanged
    ...
    void ScheduleMainThread(std::function<bool()> wait, std::function<void()> callback);
    ...
    void SetOnEnable(TransformWrapper object, std::function<void()> callback, bool once = false);
    ...
    /// @param once If the callback should only run once then be removed
    void SetOnDisable(TransformWrapper object, std::function<void()> callback, bool once = false);
